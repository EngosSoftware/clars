...
fn clar() -> Clar {
  Clar::new(APP).options_commands(
    [ClarOption::new("help", 'h', "help").standalone(), ClarOption::new("color", 'c', "color")],
    [
      ClarCommand::new("a").options([ClarOption::new("help", 'h', "help").standalone()]),
      ClarCommand::new("b").options([ClarOption::new("verbose", 'v', "verbose").standalone()]),
      ClarCommand::new("c"),
    ],
  )
}
...
#[test]
fn _0002() {
  let matches = clar().resolve(["-c", "b"]).unwrap();
...
#[test]
fn _0003() {
  assert_eq!("option '-h' must be used alone", clar().resolve(["-h", "-V"]).unwrap_err().to_string());
}
...
#[test]
fn _0007() {
  assert_eq!("option '-h' must be used alone", clar().resolve(["a", "-h", "-v"]).unwrap_err().to_string());
}
